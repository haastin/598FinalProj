...
class MemmapDataset(Dataset):
    def __init__(self, memmap_path, shape, label_memmap_path, label_shape, indices=None, transform=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        image = transforms.functional.to_pil_image(self.images[actual_idx])
...
def test_loop(dataloader, model, loss_fn, writer):
    ...
    num_classes = 63  # Adjust based on your dataset classes
...
if __name__ == "__main__":
    ...
    images_shape = (dataset_size, 224, 224, 3)
    labels_shape=(dataset_size,) 
    ...
    images_memmap_path = '/scratch/ccui17/images_val.memmap'
    labels_memmap_path = '/scratch/ccui17/labels_val.memmap'
    ...
    test_dataset = MemmapDataset(images_memmap_path, images_shape, labels_memmap_path,labels_shape,indices=test_indices, transform=transform)
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size)


    total_pics_in_training_dataset = 0

    for i, portion in enumerate(data_portions):

        model = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=False)
        num_features = model.fc.in_features
        model.fc = nn.Linear(num_features, 63)  # Change the last layer to 63 classes
        model = model.cuda()
        ...
        train_dataset = MemmapDataset(images_memmap_path, images_shape, labels_memmap_path,labels_shape,indices=train_indices, transform=transform)
